...
function setup() {
	// P5 Setup Fcn, must be called for Anim to work.
	let sz = g_canvas.cell_size;
	let width = sz * g_canvas.wid;
	// Our 'canvas' uses cells of given size, not 1x1 pixels.
	let height = sz * g_canvas.hgt;
	createCanvas(width, height);
	// Make a P5 canvas.
	draw_grid(10, 50, 'white', 'yellow');
	// Calls fcn in another (loaded) file.
}
var g_bot = { dir:3, x:20, y:20, color:100 }; // Here are some more simple multi-slot objects we use.
var g_box = { t:1, hgt:47, l:1, wid:63 };	  	 // Dir is 0..7 clock, w 0 up.
															 // Box in which bot can move.

function move_bot() {
	let dir = (round (8 * random()))
...
function get_rgb(cpix) {
	// Get RGB integer color at canvas pixel pos.
...
function paint_cell_interior(cell) {
	// Paint grid-cell insides, with pre-set color.
...
function draw_bot() {
	// Convert bot pos to grid pos & draw bot cell.
	let sz = g_canvas.cell_size;
	...
	if (0 != cpix_rgb) {
		...
		fill(0);
		stroke(0);
	} else { // Else none, Bot is visiting, so color border white.
		stroke('white');
	} 
...
function draw_update() {
	// Update our display, move & draw bot cell.
	//	 Update our display, move & draw bot cell.
...
function draw() {
	// P5 Frame Anim-draw Fcn, Called for Every Frame at Frame-rate.
	++	g_frame_cnt;
	// Count each P5 frame-draw request.
...
function keyPressed() {
	// P5 fcn, called for every keypress.
	// Any keypress, we don't distinguish. See P5 docs for using keys.
	g_stop = ! g_stop; // Toggle the bot move-paint on/off.
}

function set_bot_pos() {
	// Update bot cell pos based on mouse's canvas pixel pos.
	// Req's cell-to-pixel tranlation.
	let x = mouseX;
	// Get P5 mouse canvas pixel coords.
	let y = mouseY;
...
function mousePressed() {
	// P5 fcn, called for every mouse-press.
	set_bot_pos();
	draw_bot();
}
